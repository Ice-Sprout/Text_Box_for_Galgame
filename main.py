...
import win32process
import psutil
from api import get_emotion_from_text, get_emotion_name, EMOTION_COUNT, DEFAULT_EMOTION
...
# 修改预生成图片函数
def generate_and_save_images(character_name, background_name=current_background): # 添加背景参数
    pre_generate_character_images(character_name, background_name)  # 传入背景名称
...
def switch_character(new_index):
    ...
    if 0 <= new_index < len(character_list):
        ...
        # 生成并保存图片 - 使用当前背景
        generate_and_save_images(character_name, current_background)  # 传入当前背景
        
        return True
    return False

# 添加背景切换函数
def switch_background(background_name):
    global current_background
    if background_name in background_configs:
        current_background = background_name
        print(f"已切换到背景: {background_name}")
        # 重新生成当前角色的预生成图片
        generate_and_save_images(get_current_character(), background_name)
        return True
    else:
        print(f"背景 '{background_name}' 不存在")
        return False
...
def get_random_value(input_text: str = None):
    ...
    num_bg = background_configs[current_background]["num_bg"]  # 使用当前背景
    ...
    if expression:
        ...
        value_1 = i
        expression = None
        return f"{character_name}_{current_background}_{i}"  # 修改文件名格式
...
# 添加背景切换热键
keyboard.add_hotkey('ctrl+b', lambda: switch_background("hoshishiro"))
keyboard.add_hotkey('ctrl+n', lambda: switch_background("pieces"))
